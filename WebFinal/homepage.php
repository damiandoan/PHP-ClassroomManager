...
session_start();
  ...
  if(isset($_POST['delete-classroom'])){
    ...
    $alert = delete_classroom($classroom_name, $email);
    header('Location: '.$_SERVER["PHP_SELF"], true, 303);
...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
  ...
  <h1  >Classes Management System</h1>
        ...
        <input oninput ="hint(this.value, '<?=$_SESSION['email']?>')" class="form-control me-2" type="search" placeholder="Find your class">
           ...
           if($stmt->execute()){
            ...
            if ($result->num_rows > 0) {
              ...
              while($row = $result->fetch_assoc()) {
                ...
                <div class="col-md-4 classroom-card" id = '<?=$row['classroom_name']?>'>
                                            ...
                                            <ul class="nav nav-pills card-header-pills">
                                              ...
                                              <li class="nav-item ml-auto">
                                                <div class="dropdown">
                                                  ...
                                                  <div class="dropdown-menu">
                                                  <button type="submit" form = 'delete-classroom' value = '<?=$email?><*><?=$row['classroom_name']?>' name ='delete-classroom' class="dropdown-item"> Delete</button>
                                                  <button type="submit" form = 'add-student' class="dropdown-item">Add student</button>
                                                  </div>
                                                </div>
                                              </li>
                                            </ul>
                                    ...
                                    <div class="card-body">
                                              <img class="card-img-top" <?php if($row['image']!=null){?>src="database/classroom_images/<?= $row['image']?>"<?php }?>alt="" >
                                              <h4 class="card-title"><?=$row['subject']?></h4>
                                              <p class="card-text"><?=$row['course_length'] ?> Lessons</p>
                                              <button  class = 'btn btn-warning' type ='submit' form ='go-to-classroom' name  = 'go-to-classroom' value ='<?=$email?><*><?=$row['classroom_name']?>' >Open classroom</button>
                                    </div>
                          </a>    
